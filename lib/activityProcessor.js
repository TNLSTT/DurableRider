...
import { generateWattsHrChart } from './chart.js';
...
function buildDescriptionBlock({
  ...
  if (chartUrl) {
    lines.push(`• Early vs late chart: ${chartUrl}`);
  } else if (process.env.S3_BUCKET_NAME) {
    lines.push('• Early vs late chart: upload pending (check S3 configuration)');
  }
...
export async function processActivity({ athleteId, activityId }) {
  ...
  try {
    ...
    let chartUrl = null;
    try {
      chartUrl = await generateWattsHrChart({
        time: streams.time,
        watts: streams.watts,
        heartrate: streams.heartrate,
    ...
    const block = buildDescriptionBlock({ metrics, baseline, hrr, cadenceSummary, chartUrl, context });
